...
const mxAnalysisSettings = Base => class extends Base {
  constructor(...params) {
    ...
    RegisterFaceManagerEvent(
      `mxanalysissettings-${AppUtils.randomHexstring()}`,
  ...
  async refreshContent() {
    ...
    [
      ...
      this.refreshCustomFormOptions(x));
    ...
    [
      ...
      this.refreshMultiselectFormGroup(x));
  ...
  refreshMultiselectFormGroup(custom) {
    const dropdown = this.parentContainer
      .find(`div.dropdown-menu[data-type=${custom.name}]`);
    ...
    dropdown
      .children('a.dropdown-item[data-value!="undefined"]')
      .remove();
    ...
    custom.options.forEach((option) => {
      ...
      if (option.canUse === false) {
        ...
        if (this.aiOptions[custom.name]
          .findIndex((a) => a === option.value) >= 0) {
  ...
  async onFaceManagerEvent(
    ...
    this.availableFaceCollections
      = await this.getAvailableFaceCollections();

    this.refreshCustomFormOptions({
      name: OPT_FACECOLLECTIONID,
      options: (this.availableFaceCollections || [])
        .map((x) => ({
          name: `${x.name} (${x.faces} faces)`,
          value: x.name,
          canUse: (x.faces > 0),
    ...
    if (currentSelected && currentSelected !== 'undefined') {
      const found = this.availableFaceCollections
        .find((x) =>
